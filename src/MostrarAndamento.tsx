...
const MostrarAndamento = ({ processoId }) => {
  ...
  useEffect(() => {
    const carregarObservacoes = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/processos/${processoId}/observacoes`);
        ...
        await Promise.all(
          (uniqueAtividadeIds as number[]).map(async (atividadeId) => {
            try {
              const response = await axios.get(`http://localhost:3000/atividades/${atividadeId}`);
