...
const ProcessoDetalhes: React.FC = () => {
  ...
  const fetchProcesso = async () => {
    try {
      ...
      const responseCliente = await axios.get(`http://localhost:3000/clientes/${response.data.id_cliente}`);
      ...
      const responseRequerida = await axios.get(
        `http://localhost:3000/partes-contrarias/${parseInt(
  ...
  const fetchTipoProcesso = async (tipoProcessoId) => {
    try {
      const response = await axios.get(`http://localhost:3000/tipos-de-processo/${tipoProcessoId}`);
  ...
  const fetchAtividades = async () => {
    try {
      const response = await axios.get("http://localhost:3000/atividades");
