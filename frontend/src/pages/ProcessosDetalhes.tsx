...
const ProcessoDetalhes: React.FC = () => {
  ...
  const handleRegistrar = async () => {
    ...
    try {
      ...
      const response = await axios.post("http://127.0.0.1:3001/registrar-andamento", data);
  ...
  const fetchProcesso = async () => {
    try {
      ...
      const responseCliente = await axios.get(`http://127.0.0.1:3001/clientes/${response.data.id_cliente}`);
      ...
      const responseRequerida = await axios.get(
        `http://127.0.0.1:3001/partes-contrarias/${parseInt(
  ...
  const fetchTipoProcesso = async (tipoProcessoId) => {
    try {
      const response = await axios.get(`http://127.0.0.1:3001/tipos-de-processo/${tipoProcessoId}`);
  ...
  const fetchAtividades = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:3001/atividades");
  ...
  const fetchEquipes = async () => {
    try {
      const response = await axios.get("http://127.0.0.1:3001/equipes");
