...
const MostrarAndamento = ({ processoId }) => {
  ...
  useEffect(() => {
    const carregarObservacoes = async () => {
      try {
        const response = await axios.get(`http://18.225.117.159:3000/processos/${processoId}/observacoes`);
        ...
        await Promise.all(
          (uniqueUsuarioIds as number[]).map(async (usuarioId) => {
            try {
              const response = await axios.get(`http://18.225.117.159:3000/usuarios/${usuarioId}`);
        ...
        await Promise.all(
          (uniqueAtividadeIds as number[]).map(async (atividadeId) => {
            try {
              const response = await axios.get(`http://18.225.117.159:3000/atividades/${atividadeId}`);
        ...
        await Promise.all(
          (uniqueEquipeIds as number[]).map(async (equipeId) => {
            try {
              const response = await axios.get(`http://18.225.117.159:3000/equipes/${equipeId}`);
              const { nome } = response.data as { nome: string };
